...
export class SysOrgEditComponent implements OnInit {
  ...
  schema: SFSchema = {
    properties: {
      ...
      title: { type: 'string', title: '机构名称', maxLength: 50, ui: { grid: { span: 8 } } },
      dictOrgTypeId:  { type: 'string',
                           title: '机构类型',
                           ui: { widget: 'select', grid: { span: 8 } },
                           default: 'null',
                           enum: []
                      },
      address: { type: 'string', title: '机构地址', ui: { grid: { span: 8 } } },
      telephone: { type: 'string', title: '电话', ui: { grid: { span: 8 } } },
      fax: { type: 'string', title: '传真', ui: { grid: { span: 8 } } },
      code: { type: 'string', title: '行政编码', ui: { grid: { span: 8 } } },
      longitude: { type: 'string', title: '经度', ui: { grid: { span: 8 } } },
      latitude: { type: 'string', title: '纬度', ui: { grid: { span: 8 } } },
      sort: { type: 'number', title: '机构顺序', ui: { grid: { span: 8 } } },
      description: { type: 'string', title: '机构描述', ui: { widget: 'textarea', autosize: { minRows: 2, maxRows: 6 } } }
    },
    required: ['title', 'dictOrgTypeId', 'telephone', 'address', 'sort'],
    ui: {
      spanLabelFixed: 120,
      grid: { span: 24 }
    }
